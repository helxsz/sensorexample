...
*****************************************************/
app.get('/admin/contacts', requireRole('admin'), function(req, res){
    var locals = {};	
    var skip = (req.query["s"])?req.query["s"]:0, limit = (req.query["l"])?req.query["l"]:10, option = {'skip':skip,'limit':limit};
	var name = req.query['n'];
    var name = (name)?{}:{};
	...
	async.parallel([
	    function(callback) {		
		        console.log('retrieve admin user page'.green, req.session.username,req.session.uid);
		        contactModel.findContacts(name,option,function(err,contacts){
		            if(err) {
			            console.log('user uid not found'.red);
		            }
			        else{
			            console.log('find contacts'.green,contacts.length);
                        locals.contacts = contacts;	
						console.log(contacts[0].email,contacts[0].sub,contacts[0].msg,contacts[0].time);
                    }
                    callback();					
		        })
	        ...
	        if (err) return next(err);
            if(req.xhr){
			    res.send(locals.contacts);				
 			    return;
			}
			
		    locals.title = 'Admin - Contacts';
			locals.page = 'contacts';			
		    res.render('admin/admin_contact', locals);
	    });	
});